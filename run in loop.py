...
for num_iter in range(7):
    ...
    def vbpr(user_count, item_count, hidden_dim=10, hidden_img_dim=10,
        ...
        visual_b = tf.get_variable("visual_b", [user_count + 1, image_feat_dim],
                                   initializer=tf.random_normal_initializer(0, 0.1))
        visual_bias = tf.get_variable("visual_bias", [1, image_feat_dim],
                                      initializer=tf.random_normal_initializer(0, 0.1))
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        all_parameters_for_analysis.to_csv(str(num_iter)+'.csv',index=False)
