...
def generate_test_cold_start(user_ratings, item_ratings):
    '''
...
def vbpr(user_count, item_count, hidden_dim=20, hidden_img_dim=128,
    ...
    visual_b = tf.get_variable("visual_bias", [item_count + 1, image_feat_dim], initializer=tf.constant_initializer(0.0))
    ...
    i_v_b = tf.nn.embedding_lookup(visual_b, i)
    j_v_b = tf.nn.embedding_lookup(visual_b, j)
...
with tf.Graph().as_default(), tf.Session() as session:
    ...
    for epoch in range(1, 15):
        ...
        user_count = 0
        auc_train_values = []
        for d, _iv, _jv in uniform_sample_batch(user_ratings, user_ratings_test_cold_start, item_count, image_features):
            user_count += 1
    ...
    user_count = 0
    print "auc ..."
    for d, _iv, _jv in test_batch_generator_by_user(user_ratings, user_ratings_test_cold_start, item_count, image_features):
        user_count += 1
        _loss, _auc = session.run([loss, auc], feed_dict={u: d[:, 0], i: d[:, 1], j: d[:, 2], iv: _iv, jv: _jv})
        _loss_test += _loss
        auc_values.append(_auc)
    print "test_loss: ", _loss_test / user_count, "cold start auc: ", numpy.mean(auc_values)
    print ""
